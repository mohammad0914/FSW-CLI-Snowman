...
function showGuessedLetter() {
  console.log("Letter you have guessed is :");
  console.log(letterGuessed.join(","));
}
...
const startGame = () => {
  ...
  initGuessedWord();
  showGuessedWord();
  let numberOfGuess = randomWord.length;
  let letter;
  while (numberOfGuess > 0) {
    letter = getValidLetterGuess();
    letterGuessed.push(letter);

    if (containLetter(letter).length === 0)
      numberOfGuess--;

    updateWord(letter);
    showGuessedWord();
    showGuessedLetter();
    if (randomWord === guessedWord.join('')) {
      console.log("Your Guess is Correct....");
      break;
    }
    console.log(`Remainig guess is:${numberOfGuess}`);
...
function updateWord(letter) {
  let array = containLetter(letter);
  array.forEach((index) => {
